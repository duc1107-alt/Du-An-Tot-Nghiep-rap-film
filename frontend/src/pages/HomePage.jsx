...
import { Film, CalendarDays, Compass, Star, ArrowUp, ChevronLeft, ChevronRight, Sparkles, Heart, Zap, Wand2, Users, Flame, Calendar, Clock } from 'lucide-react';
    ...
    <div className="space-y-12 pb-16 relative">
      ...
      {bestSellers && bestSellers.length > 0 && (
        <div className="space-y-6 animate-fade-in">
          ...
          <div className="flex overflow-x-auto pb-4 gap-6 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent snap-x md:grid md:grid-cols-5 md:overflow-x-visible md:pb-0">
            {bestSellers.map((movie, index) => {
              ...
              return (
                <div 
                  key={movie._id} 
                  className="snap-start min-w-[200px] w-[200px] md:w-auto flex-shrink-0 group relative rounded-2xl overflow-hidden border border-gray-100 hover:border-brand/40 bg-white shadow-md transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_15px_30px_rgba(200,135,43,0.12)]"
                >
                  {/* Rank Overlay Badge */}
                  ...
                  <div className="absolute bottom-0 inset-x-0 p-4 flex flex-col justify-end min-h-[120px] bg-gradient-to-t from-black/90 via-black/45 to-transparent">
                    {/* Rating and Genre */}
                    <div className="flex items-center gap-1.5 mb-1.5 flex-wrap">
                      ...
                      {movie.genre && movie.genre.slice(0, 1).map((g) => (
                        <span key={g} className="text-[9px] text-gray-300 font-medium">
                          {t(g)}
                        </span>
                      ))}
                    </div>
                    ...
                    {/* Year and Duration */}
                    <div className="mt-2 pt-2 border-t border-white/10 flex items-center justify-between text-[11px] text-gray-300 font-medium">
